...
Smart installation script to auto-detect hardware and install PyTorch with required dependencies.
"""
import subprocess
import sys
import platform
...
def smart_install():
    ...
    if run_command(torch_cmd):
        ...
        run_command(f"-r {req_path}")
        
        # Write success marker
        venv_dir = os.path.dirname(os.path.dirname(sys.executable))
        try:
            with open(os.path.join(venv_dir, ".setup_complete"), "w") as f:
                f.write("done")
        except:
            pass
