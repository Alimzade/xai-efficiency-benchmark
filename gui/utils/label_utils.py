...
Provides helper functions for mapping model predictions to ImageNet class names dynamically.
"""
import torch
...
def get_label_mapping(model_name, predicted_class, label, label_names):
    ...
    if model_name in IMAGENET_LABELS and len(IMAGENET_LABELS[model_name]) > 0:
        try:
