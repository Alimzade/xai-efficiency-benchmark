...
Optional high-fidelity ReportLab-based PDF report compiler.
Generates polished landscape PDF benchmark reports.
"""
import os
...
def generate_reportlab_pdf(batch_id, results_data, selected_methods, output_path, total_time=0, environment=None, benchmark_settings=None):
    """
    Compiles a ReportLab PDF report.
    - Page 1: Environment & Benchmark Configuration Metadata + Performance Summary Table
    - Pages 2+: Per-Image & Model Architecture Evaluation Pages (Heatmap Collage + Metrics Table)
    """
    ...
    # =========================================================================
    story.append(Paragraph(f"XAI Efficiency Benchmark — Summary Report", title_style))
    ...
    # Try getting from Streamlit session state if running live
    if not methods_info and 'st' in globals() and hasattr(st, "session_state"):
        methods_info = st.session_state.get("current_batch_methods_info", [])

    # Automatic fallback: reconstruct parameter configurations directly from methods_list
    if not methods_info and methods_list:
        try:
            ...
            try:
                from gui.utils.loader import expand_xai_methods_with_params
                methods_info = expand_xai_methods_with_params(methods_list)
            except Exception:
                pass
    ...
    if not fdf_temp.empty:
        ...
        try:
            from backend.exporter import normalize_metric_columns, ATTR_RUNTIME_COL, ATTR_MEMORY_COL
        except ImportError:
            from gui.backend.exporter import normalize_metric_columns, ATTR_RUNTIME_COL, ATTR_MEMORY_COL
    ...
    if not fdf_temp.empty:
        ...
        try:
            from components.plots import (
                ...
                plot_image_size_runtime_scaling,
                plot_image_size_memory_scaling,
    ...
    for group in results_data:
        ...
        architectures = list(dict.fromkeys([m["model"] for m in group["models"]]))
        
        for arch in architectures:
            arch_models = [m for m in group["models"] if m.get("model") == arch]
                ...
                for m_data in arch_models:
                    ...
                    if res:
                        ...
                        if os.path.exists(h_p):
                            lbl_text = f"<b>{res['Method']}</b> ({m_data['input_size']}px)"
            ...
            try:
                from backend.exporter import presentation_df
            except ImportError:
                from gui.backend.exporter import presentation_df
            arch_results = [r for method in selected_methods for m in arch_models for r in [next((x for x in m["results"] if x["Method"].lower() == method.lower()), None)] if r]
            if arch_results:
