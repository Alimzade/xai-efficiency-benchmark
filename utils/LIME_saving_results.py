...
def get_next_run_number(method, n_segments, model, dataset, gpu_name):
        ...
        with open(summary_file, 'r') as f:
            for line in f:
                ...
                if (entry.get("method") == method and
                    entry.get("n_segments") == n_segments and
...
def save_experiment_results(method, n_segments, model, dataset, summary, per_image_results, avg_values, std_devs, n_samples, compactness, sigma):  
    os.makedirs("experiment_results", exist_ok=True)
    
    # Determine the run number once
    run_number = get_next_run_number(method, n_segments, model, dataset, summary.get("gpu_name", "unknown_gpu"))

    # Append to the summary file
    summary_file = "experiment_results/LIME/summary.jsonl"
    ...
    per_image_results_dir = "experiment_results/LIME/per_image_results"
    ...
    per_image_results_file = (
        f"{per_image_results_dir}/{method}_{n_segments}s_{n_samples}n_{compactness}c_{sigma}sig_"
        f"{model}_{dataset}_{gpu_name}_run{run_number}.csv"
    )
    # Save per-image results to the CSV file
    per_image_df = pd.DataFrame(per_image_results)
    per_image_df["method"] = method
    per_image_df["n_segments"] = n_segments
    per_image_df["n_samples"] = n_samples
    per_image_df["compactness"] = compactness
    per_image_df["sigma"] = sigma
